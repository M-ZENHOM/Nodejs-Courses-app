...
export default function UpdateCourse({ id }: { id: string }) {
    ...
    const { error, data } = useQuery({
        ...
        queryFn: async () => await axios.get(`http://localhost:5000/api/courses/${id}`).then(res => res.data.data.course)
    })
    async function onSubmit(values: z.infer<typeof UpdateCourseSchema>) {
        try {
            await axios.patch(`http://localhost:5000/api/courses/${id}`, { ...values, })
